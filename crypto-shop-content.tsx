...
import { Copy, PieChart, AlertTriangle, PlusCircle, Gift, Sparkles, Bot, LoaderCircle, Lightbulb, Info, ShoppingCart, ArrowRight } from 'lucide-react';
...
const CryptoIcon = ({ symbol }: { symbol: string }) => {
  ...
  const coinData = cryptoData.find(c => c.symbol.toUpperCase() === symbol.toUpperCase());
...
function PurchaseSingleCoinModal({
  ...
  useEffect(() => {
    // Reset amount when modal opens for a new coin
    if (isOpen) {
      setAmountUSD('');
    }
  }, [isOpen]);
  ...
  const estimatedQuantity = parseFloat(amountUSD) > 0 && coin.price > 0 
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      const result = await createPurchase(db, user.uid, user.email, singleCoinBundle);

      if (result.success) {
        toast({
          title: "Purchase Request Submitted!",
          description: "Your request has been received. Our team will process it shortly. The assets will appear in your portfolio upon completion."
        });
      } else {
       ...
       toast({
        ...
        description: 'Could not save your purchase record. Please try again.',
      });
    } finally {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-2xl">Buy {coin.name} ({coin.symbol})</DialogTitle>
          <DialogDescription>
        ...
        <div className="grid gap-4 py-4">
          ...
          {estimatedQuantity > 0 && (
             <p className="text-sm text-muted-foreground">
              You will receive approx. <span className="font-bold text-primary">{estimatedQuantity.toLocaleString('en-US', { maximumFractionDigits: 8 })} {coin.symbol}</span>.
            </p>
          )}
           <Alert>
              ...
              <AlertDescription>
                Upon confirmation, ${amountUSD || 0} will be deducted from your available USDT balance. If your balance is insufficient, our team will contact you.
              </AlertDescription>
          </Alert>
          <div className="flex items-start space-x-2 rounded-lg border border-yellow-500/50 bg-yellow-500/10 p-3 text-yellow-300">
            ...
            <p className="text-xs">
              The final amount of crypto received depends on market prices at the time of execution (slippage). Our team always strives to minimize this. All transactions are final.
            </p>
          </div>
        </div>
        <DialogFooter>
          ...
          <Button type="submit" onClick={handleProceed} disabled={isProcessing || !amountUSD || parseFloat(amountUSD) <= 0}>
            {isProcessing ? 'Processing...' : `Purchase for $${amountUSD || '0'}`}
            </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function PurchaseModal({
  ...
  const handleProceed = async () => {
    ...
    setIsProcessing(true);
    try {
      const result = await createPurchase(db, user.uid, user.email, bundle);
      if (result.success) {
        toast({
          title: "Purchase Request Submitted!",
          description: "Your request has been received. Our team will process it shortly. The assets will appear in your portfolio upon completion."
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="grid gap-4 py-4">
           <Alert>
              ...
              <AlertTitle>Internal Processing</AlertTitle>
              <AlertDescription>
                Upon confirmation, ${bundle.price} will be deducted from your available USDT balance. If your balance is insufficient, our team will contact you with options to complete the deposit.
              </AlertDescription>
          </Alert>
          <div className="flex items-start space-x-2 rounded-lg border border-yellow-500/50 bg-yellow-500/10 p-3 text-yellow-300">
            <AlertTriangle className="h-5 w-5 mt-0.5" />
            <p className="text-xs">
              The final amount of crypto received depends on market prices at the time of execution. Due to market volatility, there may be a small difference (slippage) between the expected and final price. Our team always strives to minimize this. All transactions are final.
            </p>
          </div>
        </div>
        <DialogFooter>
...
function BundleCard({
  ...
  return (
    <Card ref={cardRef} className={cn("flex flex-col transition-all duration-300", isRecommended ? "border-primary ring-2 ring-primary shadow-lg" : "")}>
      <CardHeader>
      ...
      <CardContent className="flex-grow">
        <div className="mb-4">
          <p className="text-sm font-medium text-muted-foreground mb-2">Contains:</p>
          <div className="flex flex-wrap gap-2">
            {bundle.coins.map((coin) => (
              <div key={coin.id} className="flex items-center gap-2 rounded-full border bg-muted px-3 py-1 text-sm">
      ...
      <CardFooter className="flex items-center justify-between">
        ...
        <Button onClick={() => onPurchase(bundle)}>Purchase Bundle</Button>
      </CardFooter>
    </Card>
  );
}

function BuildersChoiceCard({ bundle, onPurchase, isRecommended }: { bundle: CryptoBundle; onPurchase: (bundle: CryptoBundle) => void; isRecommended?: boolean }) {
  const [selectedCoinId, setSelectedCoinId] = useState(buildersChoiceOptions[0].id);
  const cardRef = useRef<HTMLDivElement>(null);
  ...
  const handlePurchase = () => {
    const selectedCoin = buildersChoiceOptions.find(c => c.id === selectedCoinId);
  ...
  return (
    <Card ref={cardRef} className={cn("flex flex-col transition-all duration-300", isRecommended ? "border-primary ring-2 ring-primary shadow-lg" : "border-primary/50")}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-2xl">{bundle.name}</CardTitle>
      ...
      <CardContent className="flex-grow">
        <div className="mb-4 space-y-4">
          <div>
            <p className="text-sm font-medium text-muted-foreground mb-2">1. Choose your main asset:</p>
          ...
          <div>
             <p className="text-sm font-medium text-muted-foreground mb-2">2. Get this bonus asset on us:</p>
             <div className="flex items-center gap-2 rounded-md border bg-muted/50 px-3 py-2 text-sm">
                <CryptoIcon symbol={'✨'} />
                <div>
                    <span className="font-semibold">A surprise bonus asset!</span>
                    <p className="text-xs text-muted-foreground">A promising crypto hand-picked by our team.</p>
                </div>
      ...
      <CardFooter className="flex items-center justify-between">
        <p className="text-3xl font-bold text-primary">${bundle.price}</p>
        <Button onClick={handlePurchase}>Purchase Bundle</Button>
      </CardFooter>
    </Card>
  );
}
...
function AiAdvisor({ onRecommendation }: { onRecommendation: (bundleId: string | null) => void }) {
  ...
  const handleGetAdvice = async (selectedGoal: 'growth' | 'gains' | 'stability') => {
    setGoal(selectedGoal);
    onRecommendation(null); // Reset recommendation in parent to remove highlight
      ...
      <CardHeader>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <CardDescription>Unsure where to start? Let our AI guide you.</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <p className="text-muted-foreground font-medium">What is your primary investment goal?</p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <Button variant={goal === 'growth' ? 'default' : 'outline'} onClick={() => handleGetAdvice('growth')} disabled={isLoading}>Long-term Growth</Button>
          <Button variant={goal === 'gains' ? 'default' : 'outline'} onClick={() => handleGetAdvice('gains')} disabled={isLoading}>Fast Gains</Button>
          <Button variant={goal === 'stability' ? 'default' : 'outline'} onClick={() => handleGetAdvice('stability')} disabled={isLoading}>Stability</Button>
        </div>
        ...
        {error && (
            <Alert variant="destructive" className="mt-4">
              ...
              <AlertDescription>{error || "The AI Advisor is currently unavailable."}</AlertDescription>
            </Alert>
        )}
...
function SingleCoinCard({ coin, price, onPurchase }: { coin: CryptoCoin, price: number | undefined, onPurchase: (coin: CryptoCoin & { price: number }) => void }) {
  const { cryptoData } = usePortfolio();
  ...
  return (
    <Card className="flex flex-col justify-between">
      <CardHeader className="flex-row items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <Image
            src={coinMarketData?.image || `https://placehold.co/40x40.png?text=${coin.symbol.slice(0,1)}`}
            alt={`${coin.name} logo`}
            width={40}
            height={40}
            sizes="40px"
      ...
      <CardContent>
        ...
        {price !== undefined ? (
          <p className="text-2xl font-bold">${price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 6 })}</p>
        ) : (
...
export function CryptoShopContent() {
  ...
  const [selectedSingleCoin, setSelectedSingleCoin] = useState<(CryptoCoin & {price: number}) | null>(null);
  const [recommendedBundleId, setRecommendedBundleId] = useState<string | null>(null);
  const { user } = useAuth();
  const { toast } = useToast();
  const { cryptoData } = usePortfolio();
  ...
  const handleSingleCoinPurchaseClick = (coin: CryptoCoin & { price: number }) => {
     if (!user) {
      toast({
        variant: 'destructive',
        title: 'Authentication Required',
  ...
  const buildersChoiceBundle = cryptoBundles.find(b => b.id === 'builders-choice-75');
      ...
      <div className="container mx-auto px-4 py-12 md:px-6">
        <header className="text-center mb-12">
          ...
          <p className="mt-4 max-w-2xl mx-auto text-lg text-muted-foreground">
            Forget the order books. Choose from expertly curated crypto bundles or buy a single asset instantly.
          </p>
        </header>
        ...
        <div className="my-12">
          <h2 className="text-3xl font-bold text-center mb-2">Invest in Curated Bundles</h2>
          <p className="text-muted-foreground text-center mb-8">Let our experts guide you with pre-packaged crypto portfolios.</p>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2">
            {cryptoBundles.filter(b => b.id !== 'builders-choice-75').map((bundle) => (
              <BundleCard key={bundle.id} bundle={bundle} onPurchase={handlePurchaseClick} isRecommended={bundle.id === recommendedBundleId} />
            ))}
            {buildersChoiceBundle && <BuildersChoiceCard bundle={buildersChoiceBundle} onPurchase={handlePurchaseClick} isRecommended={buildersChoiceBundle.id === recommendedBundleId} />}
          </div>
        </div>
        ...
        <div className="relative my-8">
            ...
            <div className="relative flex justify-center">
              <span className="bg-background px-4 text-lg font-medium text-muted-foreground">OR</span>
            </div>
        </div>

         <div className="my-12">
          <h2 className="text-3xl font-bold text-center mb-2">Buy Single Assets</h2>
          <p className="text-muted-foreground text-center mb-8">Purchase any of our supported cryptocurrencies directly.</p>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {buildersChoiceOptions.filter(c => c.symbol !== 'USDT').map((coin) => (
              <SingleCoinCard 
                key={coin.id} 
                coin={coin} 
