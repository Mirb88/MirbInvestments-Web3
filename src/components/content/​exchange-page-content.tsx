  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!asset || !amount || parseFloat(amount) <= 0) {
      toast({
        ...
        description: 'Please select an asset and enter a valid positive amount.',
      });
      return;
    }
    setIsLoading(true);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <Alert>
            ...
            <AlertDescription>
              ...
              <Button variant="link" asChild className="p-0 h-auto ml-1 font-semibold">
                <Link href={`${ROUTES.DASHBOARD}#live-market-prices`}>
          ...
          >
            <Label
              htmlFor="sell"
              className={cn(
                'flex h-full w-full flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground cursor-pointer transition-all',
                exchangeType === 'sell' && 'border-primary text-primary font-semibold'
              )}
            >
            ...
            <Label
              ...
              className={cn(
                'flex h-full w-full flex-col items-center justify-center rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground cursor-pointer transition-all',
                exchangeType === 'buy' && 'border-primary text-primary font-semibold'
              )}
            >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Select
                ...
                disabled={isLoading}
              >
                <SelectTrigger id="asset-select" aria-label="Select asset to exchange">
        ...
        <CardHeader className="flex flex-col items-center gap-4 text-center md:flex-row md:text-left md:justify-between">
          ...
          <Button asChild className="mt-4 w-full md:w-auto md:mt-0 md:flex-shrink-0">
            <Link href={`${ROUTES.SUPPORT}#direct-contact-options`}>
              Express Exchange Support <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </Button>
        </CardHeader>
      </Card>
