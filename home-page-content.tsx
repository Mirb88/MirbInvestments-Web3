...
const AiTeaserSkeleton = ({ icon: Icon, title }: { icon: React.ElementType, title: string }) => (
...
const AiExplainerTeaser = dynamic(() => import('@/components/content/home/ai-explainer-teaser').then(mod => mod.AiExplainerTeaser), {
  loading: () => <AiTeaserSkeleton icon={Wand2} title="Understand Any Crypto Concept" />,
  ssr: false,
});

const AiAdvisorTeaser = dynamic(() => import('@/components/content/home/ai-advisor-teaser').then(mod => mod.AiAdvisorTeaser), {
          ...
          <Card className="group relative overflow-hidden border-primary/30 transition-all hover:border-primary hover:shadow-[0_0_30px_hsl(var(--primary)_/_0.2)] flex flex-col min-h-[20rem]">
            ...
            <CardFooter>
              <Button asChild variant="outline" className="w-full group-hover:bg-primary group-hover:text-primary-foreground">
                <Link href={ROUTES.CLUB}>Explore Club Benefits <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
            </CardFooter>
          </Card>
          ...
          <Card className="group relative overflow-hidden border-blue-400/30 transition-all hover:border-blue-400 hover:shadow-[0_0_30px_hsl(214_96%_66%_/_0.2)] flex flex-col min-h-[20rem]">
            ...
            <CardFooter>
              <Button asChild variant="outline" className="w-full group-hover:bg-blue-400 group-hover:text-white">
                <Link href={`${ROUTES.AI_INSIGHTS}/analysis-capital-protection-2026`}>Read the Analysis <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
            </CardFooter>
          </Card>
          ...
          <Card className="group relative overflow-hidden border-accent/50 transition-all hover:border-accent hover:shadow-[0_0_30px_hsl(var(--accent)_/_0.2)] flex flex-col min-h-[20rem]">
            ...
            <CardFooter>
              <Button asChild variant="outline" className="w-full group-hover:bg-accent group-hover:text-accent-foreground">
                <Link href={ROUTES.SUPPORT}>Get the 2026 Realization Plan <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
            </CardFooter>
          </Card>
        </div>
      ...
      >
        ...
        <div className="container relative mx-auto px-4 text-center md:px-6 z-10 max-w-4xl">
          <div className="max-w-5xl mx-auto space-y-8 md:space-y-12">
            ...
            <motion.div variants={heroVariants} custom={0} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-primary/30 bg-primary/10 text-primary text-xs font-bold tracking-[0.2em] uppercase">
            ...
            >
              Elevating Luxury Assets through <span className="text-white font-semibold italic">Neural Diagnostics</span> and Purposeful Blockchain Innovation.
            </motion.p>
            ...
            <motion.div variants={heroVariants} custom={3}>
              <div className="w-full flex flex-col items-center mt-14 mb-20 px-4">
                <Button 
                  ...
                  "
                >
                  <Link href={ROUTES.CLUB} className="flex items-center justify-center w-full h-full gap-4">
