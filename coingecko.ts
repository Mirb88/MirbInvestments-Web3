...
export async function getCryptoData(ids: string[]): Promise<{ data: CryptoData[] | null; error: string | null; }> {
  try {
    ...
    console.error('Error in getCryptoData (client-side):', error);
 ...
 * @param ids - A comma-separated string of CoinGecko coin IDs.
 * @returns A promise that resolves to a price object or null.
 */
export async function getSimpleCoinPrices(ids: string): Promise<{ [key: string]: { usd: number } } | null> {
    const COINGECKO_API_URL = 'https://api.coingecko.com/api/v3';
    try {
        if (!ids) return null;

        const response = await fetch(`${COINGECKO_API_URL}/simple/price?ids=${ids}&vs_currencies=usd`, {
        ...
        console.error("Error in getSimpleCoinPrices:", err);
        return null;
    }
}
